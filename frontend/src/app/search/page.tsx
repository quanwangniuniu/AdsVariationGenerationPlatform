...
export default function AdSearchPage() {
  ...
  useEffect(() => {
    let mounted = true;
  (async () => {
    const authed = await verifySession().catch(() => false);
    if (!mounted) return;
    setIsAuthed(authed);
    if (!authed) {
...
if (!isAuthed) return null;
  ...
  return (
    <>
      <Navbar />
      <main className="p-8">
        <h1 className="text-2xl font-semibold mb-4">Ad Search</h1>
        <p className="text-gray-600">
          This is a placeholder for the Ad Search interface.
              ...
              >
                ...
                <div className="absolute inset-0">
                  <div>
                    ...
                    <button
                      onClick={() => {
                        router.push(`/ai_chat?ad_id=${item.ad_creative_id}`)
                      }}
                    >
                      Create Ads
                    </button>
